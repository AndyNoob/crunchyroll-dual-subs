...
export function showStreamLimitNotice(blockedUntil: number) {
  ...
  setTooltipText(subtitleControl!, "Crunchyroll stream limit timer (sorry)");
  subtitleMenu!.innerHTML = "";
  ...
  async function render() {
    ...
    if (remainingMs <= 0) {
      setTooltipText(subtitleControl!, "Select secondary subtitles");
      clearInterval(cooldownTimer);
