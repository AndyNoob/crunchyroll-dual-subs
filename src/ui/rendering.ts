...
import {computeState, convertToPixels, findOverlap, type RectState} from "@andynoob/move-it";

export let videoEl: HTMLVideoElement;

let otherRender: ASS | null = null;
let otherRenderObserver: MutationObserver = new MutationObserver(handleASSMask);
...
export async function beginRender(tracks: Tracks) {
  ...
  for (let [_, value] of Object.entries(tracks)) {
    ...
    otherRenderObserver.observe(overlayCanvasContainer);
...
export async function shutdownRender() {
  if (otherRender) {
    otherRender.destroy();
    otherRenderObserver.disconnect();
...
async function handleASSMask(list: MutationRecord[]) {
  ...
  for (const mutation of list) {
    ...
    if (!target.matches(".ASS-dialogue")) continue;

    for (let rect of mask.rects as RectState[]) {
      if (rect.usePercent)
        rect = convertToPixels(overlayCanvasContainer, rect);

      const overlap = findOverlap(computeState(target), rect);

      if ((mask.inverted && overlap) || (!mask.inverted && !overlap)) {
        break;
      }
